...
const MyApplicationCalendarDay = ({
  ...
  return (
    ...
    >
      <div
        className={`h-5 w-6 justify-center text-center text-xl leading-4 font-normal text-gray-950 ${isPastDate && 'opacity-40'}`}
      >
        {formatDate(date)}
      </div>
      {hasApplication && (
        <div
          className={`h-[5px] w-[5px] rounded-full ${isPastDate && 'opacity-40'}`}
        />
      )}
    </div>
  );
