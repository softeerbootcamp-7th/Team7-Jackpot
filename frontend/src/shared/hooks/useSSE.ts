...
import { SSE_MESSAGE_TYPE } from '@/shared/constants/sse';
...
export const useSSE = () => {
  ...
  useEffect(() => {
    ...
    port.onmessage = (e: MessageEvent) => {
      if (e.data.type === SSE_MESSAGE_TYPE.NOTIFICATION) {
        queryClient.invalidateQueries({ queryKey: ['notification'] });
      }
    };

    // Worker에 시작 신호 및 토큰 전달
    port.postMessage({ type: SSE_MESSAGE_TYPE.START, payload: { token } });
    // 새로고침, 탭 닫힘 감지 이벤트 추가
    const handleBeforeUnload = () => {
      port.postMessage({ type: SSE_MESSAGE_TYPE.STOP });
    };
